...
const onDelete = async () => {
  try {
    ...
    if (passwordConfirmation.value) {
      ...
      const result = await axios.delete(`/v1/machines/${route.params.id}`)
      if (result.status === 204) {
        router.push('/machine')

        notification('', 'Delete machine data success', { type: TypesEnum.Success })
      }
    } else {
      notification('Authentication Failed', 'Your password is incorrect', { type: TypesEnum.Warning })
    }
...
<template>
  <div class="main-content-container">
    <div class="main-content-header">
      ...
      <component
        :is="BaseBreadcrumb"
        :breadcrumbs="[{ name: 'Machine', path: '/machine' }, { name: route.params.id.toString() }]"
      />
    </div>
      ...
      <div class="card card-template">
        <div class="card-header bg-slate-200 dark:bg-slate-700 px-4 py-2 -mx-4 -my-2 font-extrabold">
          ...
          <div class="flex gap-2 overflow-x-hidden">
            <div>
              <router-link to="/machine/create" class="btn btn-secondary btn-sm rounded-none space-x-1">
            ...
            <div>
              <router-link
                :to="`/machine/${route.params.id}/edit`"
