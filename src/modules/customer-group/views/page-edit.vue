...
const onSubmit = async () => {
  try {
    ...
    const response = await axios.patch(`/v1/customer-groups/${_id.value}`, form.value)
  ...
  <div class="main-content-container">
    <div class="main-content-header">
      ...
      <component
        :is="BaseBreadcrumb"
        :breadcrumbs="[
          { name: 'Customer Group', path: '/customer-group' },
          { name: route.params.id.toString(), path: `/customer-group/${route.params.id.toString()}` },
          { name: 'Edit' }
        ]"
      />
    </div>
