...
const itemCategories = ref<CustomerGroupInterface[]>([])

const getItemCategories = async (page = 1, search = '') => {
  const result = await axios.get('/v1/customer-groups', {
    params: {
      pageSize: 10,
      page: page,
      sort: 'name',
...
<template>
  <div class="main-content-container">
    <div class="main-content-header">
      ...
      <component :is="BaseBreadcrumb" :breadcrumbs="[{ name: 'Customer Group' }]" />
    </div>
        ...
        <div class="flex flex-col gap-4">
          <div class="w-full flex items-center gap-4">
            <div class="w-full flex space-x-2">
              <router-link to="/customer-group/create" class="btn btn-secondary rounded-none space-x-1">
            ...
            <table class="table">
              ...
              <tbody>
                <template v-if="itemCategories.length > 0">
                  <tr v-for="CustomerGroup in itemCategories" :key="CustomerGroup._id" class="basic-table-row">
                    <td class="basic-table-body">
                      <router-link :to="`/customer-group/${CustomerGroup._id}`" class="text-info">{{
                        CustomerGroup.name
                      }}</router-link>
                    </td>
                  </tr>
