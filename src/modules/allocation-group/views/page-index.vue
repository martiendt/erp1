...
const itemCategories = ref<AllocationGroupInterface[]>([])

const getItemCategories = async (page = 1, search = '') => {
  const result = await axios.get('/v1/allocation-groups', {
    params: {
      pageSize: 10,
      page: page,
      sort: 'name',
...
<template>
  <div class="main-content-container">
    <div class="main-content-header">
      ...
      <component :is="BaseBreadcrumb" :breadcrumbs="[{ name: 'Allocation Group' }]" />
    </div>
        ...
        <div class="flex flex-col gap-4">
          <div class="w-full flex items-center gap-4">
            <div class="w-full flex space-x-2">
              <router-link to="/allocation-group/create" class="btn btn-secondary rounded-none space-x-1">
            ...
            <table class="table">
              ...
              <tbody>
                <template v-if="itemCategories.length > 0">
                  <tr v-for="AllocationGroup in itemCategories" :key="AllocationGroup._id" class="basic-table-row">
                    <td class="basic-table-body">
                      <router-link :to="`/allocation-group/${AllocationGroup._id}`" class="text-info">{{
