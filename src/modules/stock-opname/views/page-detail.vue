...
onMounted(async () => {
  try {
    const result = await axios.get(`/v1/stock-opnames/${route.params.id}`)
...
<template>
  <div class="main-content-container">
    <div class="main-content-header">
      ...
      <component
        :is="BaseBreadcrumb"
        :breadcrumbs="[{ name: 'Stock Opname', path: '/stock-opname' }, { name: route.params.id.toString() }]"
      />
    </div>
      ...
      <div class="card card-template">
        <div class="card-header bg-slate-200 dark:bg-slate-700 px-4 py-2 -mx-4 -my-2 font-extrabold">
          ...
          <div class="flex gap-2 overflow-x-hidden">
            <div>
              <router-link to="/stock-opname/create" class="btn btn-secondary btn-sm rounded-none space-x-1">
