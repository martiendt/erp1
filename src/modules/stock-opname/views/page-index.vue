...
const getTransferItems = async (page = 1, search = '') => {
  const result = await axios.get('/v1/stock-opnames', {
    params: {
      pageSize: 10,
      page: page,
      sort: 'item.name',
...
<template>
  <div class="main-content-container">
    <div class="main-content-header">
      ...
      <component :is="BaseBreadcrumb" :breadcrumbs="[{ name: 'Stock Opname' }]" />
    </div>
        ...
        <div class="flex flex-col gap-4">
          <div class="w-full flex transferItems-center gap-4">
            <div class="w-full flex space-x-2">
              <router-link to="/stock-opname/create" class="btn btn-secondary rounded-none space-x-1">
              ...
              <tbody>
                <template v-if="transferItems.length > 0">
                  <tr v-for="transferItem in transferItems" :key="transferItem._id" class="basic-table-row">
                    ...
                    <td class="basic-table-body">
                      <router-link :to="`/stock-opname/${transferItem._id}`" class="text-info">
