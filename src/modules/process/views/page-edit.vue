...
const onSubmit = async () => {
  try {
    ...
    const response = await axios.patch(`/v1/processes/${_id.value}`, form.value)

    if (response.status === 204) {
      router.push('/process/' + route.params.id)
    ...
    <div class="main-content-header">
      ...
      <component
        ...
        :breadcrumbs="[
          ...
          { name: route.params.id.toString(), path: `/process/${route.params.id.toString()}` },
          { name: 'Edit' }
        ]"
      />
    </div>
