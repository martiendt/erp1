...
const processes = ref<ProcessInterface[]>([])

const getProcesses = async (page = 1, search = '') => {
  const result = await axios.get('/v1/processes', {
    params: {
      pageSize: 10,
      page: page,
      sort: 'name',
  ...
  processes.value = result.data.data
...
watchDebounced(
  ...
  async () => {
    ...
    await getProcesses(1, searchAll.value)
...
onMounted(async () => {
  ...
  await getProcesses(page, searchAll.value)
})
...
const paginatePrev = async () => {
  ...
  await getProcesses(pagination.previousPage(), searchAll.value)
}
const paginateNext = async () => {
  ...
  await getProcesses(pagination.nextPage(), searchAll.value)
}
...
<template>
  <div class="main-content-container">
    <div class="main-content-header">
      ...
      <component :is="BaseBreadcrumb" :breadcrumbs="[{ name: 'Process' }]" />
    </div>
        ...
        <div class="flex flex-col gap-4">
          <div class="w-full flex items-center gap-4">
            <div class="w-full flex space-x-2">
              <router-link to="/process/create" class="btn btn-secondary rounded-none space-x-1">
