...
const allocations = ref<AllocationInterface[]>([])
...
const getAllocations = async (page = 1, search = '') => {
  ...
  allocations.value = result.data.data
...
watchDebounced(
  ...
  async () => {
    ...
    await getAllocations(1, searchAll.value)
...
onMounted(async () => {
  ...
  await getAllocations(page, searchAll.value)
})
...
const paginatePrev = async () => {
  ...
  await getAllocations(pagination.previousPage(), searchAll.value)
}
const paginateNext = async () => {
  ...
  await getAllocations(pagination.nextPage(), searchAll.value)
}
...
<template>
  <div class="main-content-container">
    <div class="main-content-header">
      ...
      <component :is="BaseBreadcrumb" :breadcrumbs="[{ name: 'Allocation' }]" />
    </div>
        ...
        <div class="flex flex-col gap-4">
          <div class="w-full flex items-center gap-4">
            <div class="w-full flex space-x-2">
              <router-link to="/allocation/create" class="btn btn-secondary rounded-none space-x-1">
            ...
            <table class="table">
              ...
              <tbody>
                <template v-if="allocations.length > 0">
                  <tr v-for="allocation in allocations" :key="allocation._id" class="basic-table-row">
                    <td class="basic-table-body">
                      <router-link :to="`/allocation/${allocation._id}`" class="text-info">{{
                        allocation.name
                      }}</router-link>
                    </td>
                  </tr>
